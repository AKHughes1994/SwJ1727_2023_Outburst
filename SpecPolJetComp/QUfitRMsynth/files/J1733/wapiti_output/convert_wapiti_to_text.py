...
def main():
    ...
    INPUT_DIR = '/home/andrewhughes/Projects/X-KAT/SwiftJ1727/new_meerkat/SpectroPol/publication_work/QU_phenom/files/J1733/wapiti_output'
    OUTPUT_DIR = '/home/andrewhughes/Projects/X-KAT/SwiftJ1727/new_meerkat/SpectroPol/publication_work/QU_phenom/files/J1733/QU_text'
    ...
    for txt_file in txt_files[:]:
        ...
        # Build output filename from MJD date+time and scan number from input filename
        date_str = date_isot.replace('-', '').replace(':', '')[:13]  # YYYYMMDDThhm
        scan_match = re.search(r'(scan\d+)', txt_file)
        scan_str = scan_match.group(1) if scan_match else ''
        output_file = f"{OUTPUT_DIR}/J1733_WAPITI_{date_str}{'_' + scan_str if scan_str else ''}.txt"
